...
| Name | Description |
| --- | --- |
| [update](#ANNarchy.inputs.TimedPoissonPopulation.TimedPoissonPopulation.update) | Set new parameters for the timed poisson input population. |

### update { #ANNarchy.inputs.TimedPoissonPopulation.TimedPoissonPopulation.update }
...
| Name     | Type       | Description                                                                                                                                                                                                                                                   | Default    |
|----------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------|
| rates    | np.ndarray | array of firing rates (list of floats or lists of numpy arrays). The first axis corresponds to the times where the firing rate should change and have the same length as `schedule`, if used. The other dimensions must match the geometry of the population. | _required_ |
| schedule | float      | list of times (in ms) where the firing rate should change.                                                                                                                                                                                                    | `None`     |
| period   | float      | time when the timed array will be reset and start again, allowing cycling over the schedule. Default: no cycling (-1).                                                                                                                                        | `None`     |