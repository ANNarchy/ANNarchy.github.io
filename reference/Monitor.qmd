...
```python
get(variables=None, keep=False, reshape=True, force_dict=False)
```
...
| reshape   | bool               | transforms the second axis of the array to match the population's geometry (default: True). Note that this does not apply for PopulationViews. **Important**: If you have used ANNarchy 4.x before, please note that this default has been changed in ANNarchy 5.0! | `True`    |
