...
`core.Projection.Projection(self, pre, post, target, synapse=None, name=None, disable_omp=True, copied=False)`
...
|---------------|--------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------|
| `pre`         | str \| ANNarchy.core.Population.Population | Pre-synaptic population (either its name or a ``Population`` object).                                                                                                                                  | _required_ |
| `post`        | str \| ANNarchy.core.Population.Population | Post-synaptic population (either its name or a ``Population`` object).                                                                                                                                 | _required_ |
| `target`      | str                                        | Type of the connection.                                                                                                                                                                                | _required_ |
| `synapse`     | ANNarchy.core.Synapse.Synapse              | A `Synapse` instance.                                                                                                                                                                                  | `None`     |
...
| Name                     | Type                                             | Description                                                                                                                                                                                                                                               | Default    |
|--------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------|
| `weights`                | float \| ANNarchy.core.Random.RandomDistribution | Synaptic values, either a single value or a random distribution object.                                                                                                                                                                                   | _required_ |
| `delays`                 | float \| ANNarchy.core.Random.RandomDistribution | Synaptic delays, either a single value or a random distribution object (default=dt).                                                                                                                                                                      | `0.0`      |
...
| `delays`                 | float \| ANNarchy.core.Random.RandomDistribution | Synaptic delay, either a single value or a random distribution object (default=dt). | `0.0`      |
...
| `weights`                | float \| ANNarchy.core.Random.RandomDistribution | Either a single value for all synapses or a RandomDistribution object.                                                                                                                                                                                    | `1.0`      |
| `delays`                 | float \| ANNarchy.core.Random.RandomDistribution | Either a single value for all synapses or a RandomDistribution object (default = dt)                                                                                                                                                                      | `0.0`      |
...
| `weights`                | float \| ANNarchy.core.Random.RandomDistribution | Either a single value for all synapses or a RandomDistribution object.                                                                                                                                                                                    | _required_ |
| `delays`                 | float \| ANNarchy.core.Random.RandomDistribution | Either a single value for all synapses or a RandomDistribution object (default = dt)                                                                                                                                                                      | `0.0`      |
| `allow_self_connections` | bool                                             | Defines if self-connections are allowed (default=False).                                                                                                                                                                                                  | `False`    |
| `force_multiple_weights` | bool                                             | If a single value is provided for ``weights`` and there is no learning, a single weight value will be used for the whole projection instead of one per synapse. Setting ``force_multiple_weights`` to True ensures that a value per synapse will be used. | `False`    |
...
| Name       | Type              | Description                                                                                                                                                                           | Default    |
|------------|-------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------|
| `weights`  | numpy.numpy.array | Numpy array (or list of lists of equal size) representing the weights. If a value is None, the corresponding synapse will not be created.                                             | _required_ |
...
| Name      | Type                                       | Description                                                                     | Default    |
|-----------|--------------------------------------------|---------------------------------------------------------------------------------|------------|
| `weights` | scipy.scipy.sparse.scipy.sparse.lil_matrix | a sparse lil_matrix object created from scipy.                                  | _required_ |
...
| Name                     | Type                                             | Description                                                                                                                                                                                                                                               | Default   |
|--------------------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| `weights`                | float \| ANNarchy.core.Random.RandomDistribution | Initial synaptic values, either a single value (float) or a random distribution object.                                                                                                                                                                   | `1.0`     |
| `delays`                 | float \| ANNarchy.core.Random.RandomDistribution | Synaptic delays, either a single value or a random distribution object (default=dt).                                                                                                                                                                      | `0.0`     |
| `force_multiple_weights` | bool                                             | If a single value is provided for ``weights`` and there is no learning, a single weight value will be used for the whole projection instead of one per synapse. Setting ``force_multiple_weights`` to True ensures that a value per synapse will be used. | `False`   |
