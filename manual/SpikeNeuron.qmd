...
ANNarchy provides standard spiking neuron models, similar to the ones defined in [PyNN](http://neuralensemble.org/docs/PyNN/reference/neuronmodels.html). Their definition (parameters, equations) are described in [Specific Neurons](/reference/index.qmd#neuron-models). The classes can be used directly when creating the populations (no need to implement them). Example:
