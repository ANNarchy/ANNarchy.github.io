...
First-order ordinary differential equations (ODE) can be solved using different numerical methods. The method can be declared globally in the `Network.config()` method and used in all ODEs of the network:
...
Event-driven integration is only available for spiking synapses with variables following linear first-order dynamics. Let's consider the following STDP synapse (see [Spiking Synapse](SpikeSynapse.qmd) for explanations):
