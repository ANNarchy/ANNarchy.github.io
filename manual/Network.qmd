...
proj.all_to_all(weights=ann.Uniform(0,1))
...
class SimpleNetwork (ann.Network):
    def __init__(self, N)
        ...
        self.proj.all_to_all(weights=ann.Uniform(0,1))
...
If you create a subclass of `Network`, you can also provide `dt` to its constructor, even if your network does not catch it. Please note that, you do not need to explictly call the constructor of Network (Network.__init__(self)), it is done automatically.
