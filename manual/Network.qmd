...
The `simulation()` method will be called over three copies of the network (in different processes). The first argument to this method **must** be an integer corresponding to the index of a network (here `idx = [0, 1, 2]`), the second **must** be a `Network` object (class [Network](/reference/Network.qmd)).
