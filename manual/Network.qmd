...
By default, the random number generators(RNG) are seeded with `time(NULL)`, so each simulation will be different. If you want to have deterministic simulations, you need to provide a fixed seed to `Network`:
...
Note that this also sets the seed of the old RNG of numpy, which are used to initialize values produced by `np.random.*`. If you use the new default RNG of numpy (`rng = np.random.default_rng()`), you will have to seed it yourself.

When `seed`is left to its default value `None`, a value is chosen with `time(NULL)` and it becomes accessible in the attribute `net.seed`.
