...
neuron = ann.Neuron(
    parameters = dict(
        tau = ann.parameter(10.0, locality='local'),
        baseline = ann.parameter(-0.1, locality='local'),
    )
)
```

For synapses, there are three different localities:
...
Note that constants are strictly global, changing their value impacts all objects using them. Changing the value of a constant can only be done through the `set()` method (before or after `compile()`):
...
See the reference for [Random Distributions](/reference/index.qmd#random-distributions) for more distributions. 


The arguments to the random distributions can be either fixed values or global parameters.
...
:::{.callout-caution}
If a global parameter is used, changing its value will not affect the generator after compilation (`ann.compile()`).
...
neuron = ann.Neuron(
    equations = [
        ann.variable('tau * dv/dt = baseline -v'),
        ann.variable('r = pos(v)'),
        # is equivalent to:
...
## Old interface with strings

Prior to version 4.9, the default interface was using multiline strings to define both parameters and variables. This interface still works, but might become deprecated in a future release.
...
When using strings, parameters are by default local, i.e. unique to each synapse in a given projection. This is the opposite to the default behavior with dictionaries. If the `postsynaptic` flag is passed after a colon ':', the parameter will be common to all synapses of a post-synaptic neuron, but can differ from one post-synaptic neuron to another. If the `projection` flag is passed, the parameter will be common to all synapses of a projection (e.g. the learning rate). `postsynaptic` declares one value per post-synaptic neuron in the projection.
...
Variables are local by default. The `init`, `min`, `max` flags and the numerical method can be passed after the colon.
