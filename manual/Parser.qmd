...
[Numerical methods](NumericalMethods.qmd), as it influences how
ODEs are solved).
...
-   Simple equations must hold only the name of the variable on the left
    ...
    `r + v = noise` are forbidden, as it would be impossible to
...
method (see [Numerical methods](NumericalMethods.qmd)).
...
It must be a single value (the same for all neurons in the population or all synapses in the projection) and should not depend on other parameters and variables. This initial value can be specifically changed after the `Population` or `Projection` objects are created (see [Populations](Populations.qmd)).
...
`v`, the new value will be compared to the `min` and `max` value, and
...
See the reference for [Random Distributions](/reference/index.qmd#random-distributions) for more distributions. For example:
