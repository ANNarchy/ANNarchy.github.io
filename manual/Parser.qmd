...
* `ann.Uniform(min, max)` generates random numbers from a uniform distribution in the range $[\text{min}, \text{max}]$.
* `ann.Normal(mu, sigma)` generates random numbers from a normal distribution with mean mu and standard deviation sigma.
