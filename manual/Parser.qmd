...
The default locality of a parameter is global:

```python
neuron = ann.Neuron(
    parameters = dict(
        tau = ann.Parameter(10.0), # or just: tau = 10.0
        baseline = ann.Parameter(-0.1)
    )
)
```

The `Parameter` dataclass allows to tell ANNarchy that the parameter has to be local, i.e. it should take one value per neuron in the population.
