...
Connector methods usually take the form:

```python
proj.connect_xxx(weights, delays, *other_parameters)
```
...
* a random distribution instance ([RandomDistribution](../reference/index.qmd#random-distributions)) allowing to sample values for each synapse.
...
```python
proj.connect_all_to_all(weights=ann.Uniform(0.0, 0.5)) 
```
...
```python
proj.connect_all_to_all(weights=1.0, force_multiple_weights=True) 
```



### connect_fixed_probability (sparse)
