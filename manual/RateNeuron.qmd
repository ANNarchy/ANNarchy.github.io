...
where $v(t)$ represents the membrane potential of the neuron, $\tau$
the time constant of the neuron, $B$ its baseline firing rate,
$r(t)$ its instantaneous firing rate, $i$ an index over all
excitatory synapses of this neuron, $w_i$ the efficiency of the synapse
with the pre-synaptic neuron of firing rate $r_{i}$.
...
on `v`), but not on unknown names. ANNarchy already defines the
...
[Projections](Projections.qmd) to see how the target is
...
[rate-coded synapse](RateSynapse.qmd).
